...
test('encrypts and decrypts an API key with a non-extractable key', async () => {
  ...
  const key = await deriveWrappingKey('a sufficiently long passphrase', Uint8Array.from(atob(record.salt), (c) => c.charCodeAt(0)));
...
test('uses unique salt and IV and rejects bad authentication', async () => {
  ...
  await assert.rejects(unlockEncryptedKey({ ...first, aad: 'wrong' as never }, passphrase), CryptoError);
