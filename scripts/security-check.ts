...
const allowedDependencies = new Set([
  'next', 'react', 'react-dom', 'dexie', '@anthropic-ai/sdk', 'mammoth',
  'fflate', '@react-pdf/renderer', 'undici',
]);
const allowedDevDependencies = new Set([
  'typescript', 'eslint', 'eslint-config-next', 'tsx', 'playwright',
...
for (const path of [...files('app'), ...files('lib'), ...files('components')]) {
  ...
  if (/\bconsole\.(log|debug|info|warn|error)\s*\(/.test(source)) fail(path, 'console output is forbidden');
  if (!['lib/keys.ts', 'lib/crypto.ts', 'lib/backup.ts'].includes(relative(root, path).replaceAll('\\', '/')) &&
    /(?:\.\s*(?:encryptedKey|plaintextKey)\b|\{\s*(?:encryptedKey|plaintextKey)\s*(?:,|\}))/.test(source)) {
    fail(path, 'key field read outside approved key handling');
  }
}

const eslint = join(root, 'eslint.config.mjs');
if (existsSync(eslint) && /react\/no-danger\s*:\s*(?:['"](?:off|warn|0)['"]|0|1)/.test(text(eslint))) {
  fail(eslint, 'react/no-danger must remain an error');
}

const routes = files('app/api').filter((path) => /[\\/]route\.(?:ts|tsx|js|jsx)$/.test(path))
  .map((path) => relative(root, path).replaceAll('\\', '/'));
if (routes.length !== 1 || routes[0] !== 'app/api/fetch-job/route.ts') {
  failures.push(`server routes must be only app/api/fetch-job/route.ts (found: ${routes.join(', ') || 'none'})`);
}
