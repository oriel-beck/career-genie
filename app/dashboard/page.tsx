...
export default function DashboardPage() {
    ...
    try {
      ...
      toast(
        error instanceof Error
          ? error.message
          : 'Analysis failed. You can still create and edit a job manually.',
        'error',
      );
  ...
  async function saveDraft(event: React.FormEvent) {
    ...
    if (!draft) return;
    const matchScore =
      Number.isInteger(draft.matchScore) && draft.matchScore >= 0 && draft.matchScore <= 100
        ? draft.matchScore
        : 0;
      ...
      <section className="page stack">
        ...
        <section className="card stack" aria-busy={loading || undefined}>
          ...
          {loading ? (
            ...
            <>
              ...
              <button type="button" className="secondary" onClick={() => void importUrl()}>
                Import URL
              </button>
              <label htmlFor="job-paste">
              ...
              <div className="button-row">
                ...
                <button
                  ...
                  className="secondary"
                  onClick={() => {
                    if (paste.trim()) setDraft(blankJob(paste, url || undefined));
                  }}
                >
          ...
          <form className="card stack" onSubmit={saveDraft}>
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => setDraft({ ...draft, matchScore: Number(event.target.value) })}
              />
            </label>
            {draft.gaps.length > 0 && (
              <>
        ...
        {visible.length ? (
          <div className="job-list">
            {visible.map((job) => (
              <JobCard key={job.id} job={job} onDeleted={() => void refresh()} />
            ))}
          </div>
        ) : (
