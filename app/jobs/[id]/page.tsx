...
export default function JobPage() {
  ...
  async function updateStatus(value: Job['status']) {
    ...
    await db.jobs.put(next); setJob(next); toast('Status saved.', 'success');
  }
  ...
  return <AppShell><section className="page stack">
    ...
    <section className="card stack" aria-busy={busy || undefined}><h1>{job.title}</h1><p>{job.company}</p>
      <label htmlFor="job-status">Application status
        <Select
          id="job-status"
          value={job.status}
      ...
      {busy ? (
        <div className="loader" role="status" aria-live="polite">
          ...
          <div className="loader-copy">
            ...
            <p className="loader-hint">Claude is rewriting your resume and cover letter for this role. This can take a minute.</p>
          </div>
        </div>
      ) : (
        <>
          ...
          <button type="button" onClick={() => void generate()}>{active ? 'Regenerate' : 'Generate'}</button>
        </>
      )}
    </section>
